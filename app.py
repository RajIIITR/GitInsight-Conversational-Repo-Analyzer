...
def clear_repo():
    try:
        ...
        if os.path.exists("db"):
            st.info("Removing database files...")
            # Try the Windows rd command first
            if os.name == 'nt':  # Windows
                os.system("rd /s /q db 2>nul")
                # If directory still exists, try rmtree with error handler
                if os.path.exists("db"):
                    shutil.rmtree("db", onerror=handle_remove_readonly)
            else:  # Unix-like systems
                shutil.rmtree("db")
...
with st.sidebar:
    ...
    - "You can ask question regarding the function or any python file role in our given repository"
    """)
